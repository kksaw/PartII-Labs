...
class CartPole:
    ...
    def __init__(self, visual=False, update=False):
        ...
        self.mu_c = 0.005     # friction coefficient of the cart
        self.mu_p = 0.000002  # friction coefficient of the pole
    ...
    def performAction(self, action = 0.0):
        ...
        self.sysvar = [[self.cart_location], [self.cart_velocity], 
        ...
        for step in range(self.sim_steps):
            ...
            self.sysvar[0].append(self.cart_location)
            self.sysvar[1].append(self.cart_velocity)
            self.sysvar[2].append(self.pole_angle)
            self.sysvar[3].append(self.pole_velocity)
            
            if self.update:
                self._render()
        
        self.sysvar = np.asarray(self.sysvar)
   ...
   # the following are graphics routines
    ...
    def _render(self):
        ...
        #plt.text(-10,-2,'cart_location: %.2f     cart_velocity: %.2f     pole_angle: %.2f     pole_velocity: %.2f' % (self.cart_location, self.cart_velocity, self.pole_angle, self.pole_velocity))
        #self.fig.show()
